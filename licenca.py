...
consulta_sql = "SELECT codigo, cliente, fantasia, cnpj, dias FROM licencas_clientes WHERE cliente LIKE %s"
if st.button('Pesquisar'):
    ...
    tb = pd.DataFrame(cliente_consulta, columns=['Codigo', 'Cliente', 'Fantasia', 'CNPJ', 'dias'])
    st.markdown('''
        ## Consulta Cliente
    ''')
    st.table(tb)
